...
To implement the group convolution,

1) duplicate the feature map $f$ with 4 rotations to form $F^+$ \\
2) duplicate the filter $w$ with 4 rotations to form $W^+$ \\
3) for each $w$ in $W^+$, perform traditional convolution on $F^+$ with permuted $W^+$
