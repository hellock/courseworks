...
\]
\begin{align*}
	Q(\theta;\theta^{(t)}) &= \mathbb{E}\{l_c(x_1,\dots,x_n,z_1,\dots,z_n,y_1,\dots,y_n;\theta)|x_1,\dots,x_n,\theta^{(t)}\} \\
	&= \sum_{k=1}^n\mathbb{E}\{\log(p_{z_k}q_{y_k}N(x_k;\mu_{z_k}^{(t)},\sigma_{y_k}^{(t)}))|x_k,\theta^{(t)}\} \\
...
\[
	\frac{\partial Q(\theta;\theta^{(t)})}{\partial\mu_j} = \sum_{k=1}^n\sum_{i=1}^l\,P(z_k=j,y_k=i|x_k,\theta^{(t)})\frac{x_k-\mu_j}{\sigma_i^2} = 0
\]
\[
...
One sequence generated by my program: 1 2 1 1 2 2 3 1 2 3
